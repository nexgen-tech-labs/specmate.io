...
Tables: `Organization`, `OrganizationMember`, `Workspace`, `User`, `WorkspaceMember`, `Team`, `TeamMember`, `TeamProject`, `WorkspaceInvite`, `Project`, `Source`, `RawRequirement`, `DraftItem`, `ReviewDecision`, `PublishedItem`, `AuditEvent`, `TraceLink`, `AiCallLog` (plus `Snapshot`, `UsagePeriod`, `AtlassianConnectInstall` from Epics 8/10).
...
- **Guided wizard** (`/workspaces/{ws}/projects/{p}/get-started`, `components/onboarding/onboarding-wizard.tsx`): three-step indicator — connect a tool (optional; deep-links to the Epic 5/6/7 mapping settings pages, skippable since generation only needs a source) → upload a source (reuses `UploadZone`, polls the new `GET .../sources/{id}` status endpoint until parsed) → generate (calls the trigger above). Sensible defaults mean no field-mapping decisions block a first run (10.10 AC). A fourth, non-indexed terminal step (`done`) follows a successful generate — see "Onboarding Flow redesign" below.
...
### Onboarding Flow redesign (Claude Design handoff)

Implements a Claude Design mockup (`organization-and-onboarding-flow/project/Onboarding Flow.dc.html`) that expands the 2-step signup into a 4-step flow and adds a success interstitial to the project wizard. Additive on top of the Issue 10.10/12.10 infrastructure above — `/api/signup`'s transaction shape (User → Organization → Workspace → OrganizationMember(OWNER) → WorkspaceMember(ADMIN)) and `checkSeatGate()`'s billing gate are unchanged.

- **Schema**: `Organization.size` (`OrgSize?`: `SOLO`/`SMALL`/`MEDIUM`/`LARGE`/`ENTERPRISE`), nullable — pre-existing/backfilled orgs have no size data, same posture as `Workspace.organizationId`. Mirrored in `apps/api/app/models.py` for schema-sync discipline even though apps/api doesn't read it.
- **4-step signup** (`apps/web/src/app/onboarding/onboarding-form.tsx`): account → **organization** (name, defaulted to `"{firstName}'s Company"`, plus company size) → workspace (unchanged, now references the org name in its copy) → **team invite** (optional email chips, Enter-to-add). `POST /api/signup` now accepts `orgName`, `orgSize`, and `teamEmails: string[]`; the org is named from `orgName` (previously defaulted to the workspace name) and carries `size`. Team emails create real `WorkspaceInvite` rows via the same shape as the standalone invite route (7-day TTL, `REVIEWER` role) — **creating** these invites is never gated (matches the existing "gate at consumption, not creation" pattern); a workspace still can't actually gain a second member without active billing, enforced at _acceptance_ by the unchanged `checkSeatGate()` in `/api/invites/[token]/accept`. "Skip for now" and "Create workspace" both submit with whatever `teamEmails` array is currently built (empty if skipped).
- **Wizard success screen**: `onboarding-wizard.tsx`'s `generate` step, on success, now shows a `done` step ("{n} items drafted", reading `stats.item_count` from the generate response) instead of routing straight into `/review` — the reviewer clicks through explicitly. The 3-step indicator is hidden on this terminal screen (it's a standalone confirmation card in the design, not a 4th indexed step).
- **Dashboard breadcrumb**: `/workspaces/{ws}` now shows `"{organization.name} / {orgSizeLabel}"` above the workspace name when the workspace has a parent org with a size set; omitted entirely for pre-hierarchy/backfilled orgs with no size. Label mapping lives in `apps/web/src/lib/org-size.ts`, shared between the signup form's `<select>` and this breadcrumb so the wording only exists once.
- **Visual system**: the mockup's palette/typography (`Geist`/`Geist Mono`, `#2447e8` cobalt, `#12151a` ink, etc.) is already the app's existing design system (`globals.css` CSS variables, `layout.tsx`'s font loading) — restyled using existing Tailwind tokens, no new fonts or colors introduced.
- **Out of scope**: the mockup's landing-page markup (superseded by the real landing page, Issue 10.11) and its connect-tool step (already matches current behavior) were not touched.
...
- **Free while solo** (amendment, supersedes the original plan-selection-at-signup design): a new Organization/Workspace is free for exactly one user — signup never touches Stripe, and every workspace defaults to `pricingTier: STARTER, subscriptionStatus: NONE` (live-verified: a fresh signup lands directly in the dashboard with no billing step). Billing is enforced at the actual moment a workspace becomes multi-user — invite **acceptance**, not invite creation (an admin can freely send invites that never get accepted) — via `checkSeatGate()` (`apps/web/src/lib/billing-gate.ts`): adding a distinct second `WorkspaceMember` is blocked with `402 BILLING_REQUIRED` unless the workspace is `ENTERPRISE` or has `subscriptionStatus` `TRIALING`/`ACTIVE`. Org-role-derived implicit access (Issue 12.11 inheritance) never creates a `WorkspaceMember` row and isn't a "seat" in the billing sense, so an org OWNER browsing any workspace in their org never trips this gate. The invite page (`/workspaces/{ws}/invite`) shows a proactive warning via `nextInviteNeedsBilling()` before an admin sends an invite that would fail at acceptance; the accept-invite button surfaces the specific `BILLING_REQUIRED` case with actionable copy. New `/workspaces/{ws}/billing` settings page is the only entry point into Stripe Checkout (`POST /api/workspaces/{ws}/billing/checkout`) or the Enterprise tier switch (`POST /api/workspaces/{ws}/billing/tier`) now that onboarding doesn't force the choice.
...
### Atlassian Connect app framework (Issue 10.2, technical piece only)

The Marketplace-listing acceptance criteria (public listing, screenshots, demo video, review approval) need a live Atlassian Developer account and a manual Marketplace submission — not built here. **The distinguishing technical piece — a Marketplace-compliant multi-tenant install framework — is built and live-verified**, as a genuinely different auth model from the existing single-tenant Jira integration (Issue 5.1's `CloudTokenConnection`, one env-configured connection for the whole SpecMate deployment). Connect apps are multi-tenant: any Jira Cloud site's admin can install directly from the descriptor, each site getting its own credential pair with no manual setup on SpecMate's side.

- **Descriptor** (`GET /atlassian-connect.json`, `apps/web/src/app/atlassian-connect.json/route.ts`): the well-known JSON Atlassian's install flow (and, later, Marketplace listing) fetches to learn the app's identity, lifecycle callback URLs, and scopes. `key` is a placeholder (`ATLASSIAN_CONNECT_APP_KEY` env var) pending real Marketplace registration.
- **Schema**: new `AtlassianConnectInstall` table — one row per installed Jira Cloud site (`clientKey` unique, `sharedSecret`, `baseUrl`), nullable `workspaceId`/`claimedAt` because Atlassian's `installed` callback fires _before_ any SpecMate workspace is known to be associated with that site.
- **Lifecycle webhooks**: `POST /api/atlassian-connect/installed` upserts by `clientKey` (a reinstall rotates in the new sharedSecret and un-cancels rather than erroring). This one is necessarily trust-on-first-use — the payload itself carries the sharedSecret to store, so there's no prior secret to verify the request against yet (a documented, Atlassian-acknowledged asymmetry in the spec). `POST /api/atlassian-connect/uninstalled` is the trusted path: JWT-verified against the _stored_ secret before anything is touched, rejecting forged/replayed uninstalls with 401. Idempotent for unknown `clientKey`s (Atlassian retries).
- **Workspace claim flow**: `GET/POST /api/workspaces/{ws}/atlassian-connect` — since install-time has no SpecMate account context, an admin later links (claims) an unclaimed install to their workspace from a settings page. Refuses to claim an install already claimed by a _different_ workspace (409).
- **QSH-verified JWT** (`apps/web/src/lib/atlassian-connect.ts`, using Atlassian's own `@atlassian/atlassian-jwt` reference library): verifies both the HMAC signature and the query-string-hash claim, which ties a token to one exact request (method + canonical path + sorted query params, excluding the `jwt` param itself) — prevents a valid token for one endpoint being replayed against another.
- **Outgoing auth — `ConnectJwtConnection`** (`apps/api/app/services/connectors/jira_auth.py`): a second `JiraConnection` protocol implementation alongside `CloudTokenConnection`, using 2-Legged JWT (Atlassian Connect's server-to-server auth — no OAuth token exchange, the app signs a short-lived JWT per request). `ConnectJwtAuth` is an `httpx.Auth` subclass (not a static tuple, since the QSH claim requires re-signing per request) with a 180s expiry. The QSH canonicalization (`_query_string_hash`) is hand-implemented in Python (no equivalent library exists) and **cross-verified byte-for-byte against the real output of the Node-side `@atlassian/atlassian-jwt` library** for the same request — both sides of this integration produce identical hashes. `get_connect_connection_for_workspace()` resolves a workspace's claimed install into a connection object, ready for `publish.py`'s `PublishGateway` to adopt.
- **Deliberately not wired into publish**: `publish.py`'s `PublishGateway.connection()` is still single-tenant (`get_jira_connection()`, no workspace argument) — making Jira publishing per-workspace-aware is a real call-site change belonging to the deferred per-workspace connection store (documented since Issue 5.1), not something to bolt on as a side effect of building the Connect install lifecycle. `get_connect_connection_for_workspace()` exists as the ready-made resolver for when that work happens.
- **Live-verified** against the real dev server and Postgres (no Atlassian account needed for this part — the lifecycle endpoints are plain HTTP + JWT, testable standalone): a real `installed` callback correctly created an unclaimed install; a correctly-signed `uninstalled` callback (JWT generated by the same `@atlassian/atlassian-jwt` library a real Jira instance would use) correctly marked it uninstalled.
- **Not built / needs a live Atlassian account**: actual Marketplace submission (screenshots, demo video, review process), the in-app "link to SpecMate workspace" UI panel referenced by the descriptor's `generalPages` module (currently a placeholder URL), and end-to-end verification against a real Jira Cloud site (only the lifecycle _mechanics_ are live-verified, not a real Atlassian-initiated install).

### Organization → Workspace → Team → User hierarchy (Issues 12.10/12.11)

The enterprise tenancy model, retrofitted **additively** onto the flat `Workspace → WorkspaceMember → User` model that Epics 1–10 were built against — no breaking rename of Workspace semantics, no changes to any `workspaceId` foreign key.

- **Schema**: new `Organization` (+ `OrganizationMember` with `OrgRole` `OWNER`/`ADMIN`), `Team`, `TeamMember`, `TeamProject` tables; `Workspace.organizationId` is a nullable FK. Nullable by design: all real workspaces have an org (signup always creates one; pre-hierarchy workspaces were **backfilled** into implicit single-workspace organizations named after them, with each workspace ADMIN promoted to org OWNER — verified 21/21 on the dev DB), but a workspace with no org is simply "no org-level grants" rather than a broken row, which also keeps every existing test fixture working unchanged.
- **Role inheritance** (12.11): org `OWNER` (billing, org settings, workspace lifecycle) and org `ADMIN` (workspace/team management, not billing) both cascade to an **effective Workspace ADMIN** in every workspace of the org, with or without a `WorkspaceMember` row. Resolution lives in `getWorkspaceMembershipForUser()` (`apps/web/src/lib/workspace-access.ts`), which now returns an `EffectiveMembership` (`role`, `via: WORKSPACE|ORGANIZATION`, `orgRole`) — the `requireWorkspaceRole()` signature every route already called is unchanged, which is what kept the retrofit's blast radius manageable.
- **Team scoping** (12.11 AC 2): a Team is a **restriction, not a grant** — workspace membership grants access as before; a non-admin member who belongs to ≥1 project-scoped team (a team with `TeamProject` rows) has visibility narrowed to the union of those teams' projects. Unscoped teams are plain groupings and never restrict; effective ADMINs are never restricted. `getAccessibleProjectIds()` computes the restriction; `requireProjectRole(workspaceId, projectId, roles)` enforces it (returning 404, not 403, for out-of-scope projects — no existence leak) and replaced the repeated `requireWorkspaceRole` + `project.findFirst` pattern across **all 17 project-scoped API routes and 8 project-scoped pages**. The workspace dashboard's project list and the workspace-wide trace search (`lib/trace.ts`, now taking an optional accessible-project filter) are scoped too; draft-item routes additionally verify item→project containment so an in-scope project URL can't smuggle an out-of-scope item id.
- **Endpoints**: `GET/POST /api/workspaces/{ws}/teams` + `PATCH/DELETE .../teams/{teamId}` (members must already be workspace members; scoped projects must be in-workspace; resurrected same-name teams come back empty); `GET/POST /api/organizations/{orgId}/workspaces` (org OWNER/ADMIN — first-ever post-signup workspace creation path, making multi-workspace orgs reachable). The full management UI is Issue #99, deliberately separate.
- **Signup** now creates Organization → Workspace → org OWNER + workspace ADMIN memberships in one transaction; the form UI is unchanged (org name defaults to the workspace name — the implicit-single-workspace-org shape, same as the backfill).
- **Billing stays on Workspace** for now: Issue 12.10's "Organization owns billing" conflicts with the already-implemented per-workspace Stripe model (Issue 10.9), and with no live Stripe account there's nothing to migrate — consolidating billing at the org level is an explicit open decision for when real pricing lands, recorded on Issue #97.
- **Tests**: `lib/hierarchy.test.ts` (org-role inheritance both roles, cross-org isolation, multi-org independent roles, team scoping incl. unscoped-team/backward-compat cases) + `api/workspaces/[workspaceId]/teams/route.test.ts` (team CRUD authorization, member/project validation, and end-to-end enforcement through a real route: scoped reviewer blocked with 404 out-of-scope, allowed in-scope, cross-project item smuggling blocked), all against real Postgres.

### Maintain & Sync (Epic 9, Issues 9.1–9.5)

Originally scoped "Phase 2 — architect now, build post-launch once there's demand signal from the first ~25 customers." Built in full ahead of that signal per explicit instruction, since the org/team hierarchy work made enterprise-readiness the priority. All five issues are additive on top of the Epic 3 (generation) and Epics 5/6/7 (publishing) infrastructure — no existing pipeline behavior changed for a project that never uploads a second version of a source.

- **Source versioning + diff (9.1)**: `Source.version`/`previousVersionId` (self-relation) — re-uploading over an existing source creates a **new** `Source` row chained to the one it supersedes, rather than overwriting; the prior version's `Source` + `RawRequirement`s stay queryable, untouched. Version detection at upload time (`apps/web/.../sources/route.ts`) is either explicit (`previousVersionId` form field) or automatic by filename similarity (strips `-v3`/`(2)`-style suffixes and compares base names, e.g. `Client-Requirements-v3.docx` → `v4` auto-links). New `SourceDiff` model + `apps/api/app/services/parsing/source_diff.py::diff_fragments()`: matches fragments by `sectionPath` first, then by normalized-text equality for moved-but-identical content, classifying each as added/removed/modified/unchanged. Computed automatically whenever a new-version Source finishes parsing. `GET /sources/{id}/diff` (+ web proxy) exposes it.
- **Targeted regeneration (9.2)**: `apps/api/app/services/generation/targeted.py::run_targeted_regeneration()` consumes 9.1's diff and, per fragment: **modified** → revises the existing traced `DraftItem`(s) via the same single-item AI-regen shape as Issue 3.9's `regenerate_item` (soft-deletes the old item, creates a new one linked by `revisionOfId`); **added** → drafts one brand-new unparented item from the fragment alone; **removed** → flags the traced item(s) (`flags.sourceRemoved`) for reviewer attention, **never auto-deletes**; **unchanged** → completely untouched, not re-sent to the AI at all. Approved items are protected from silent supersession — flagged (`flags.sourceModified`) instead of rewritten out from under a signed-off reviewer. `POST /sources/{id}/targeted-regenerate` (+ web proxy). Live-verified with a real Claude call: correctly picked up an added "3DS" detail into a revision and independently drafted a real refund story from an added fragment.
- **Delta review queue (9.3)**: `DraftItem.sourceVersionId` tags every item 9.2 touches with the triggering source version — the filter key for `/workspaces/{ws}/projects/{p}/sources/{sourceId}/delta-review`, which shows _only_ those items, never the full project backlog. Fully reuses the Epic 4 `ReviewQueue` component (same approve/reject/edit mechanism) rather than rebuilding one; adds a `deltaContext` field per item showing why it's here, the exact changed source-fragment text, and — for revisions — the previous version's title/description side-by-side via the `revisionOfId` chain. Sources list surfaces "Regenerate delta" / "Review delta →" once a new version's diff is ready.
- **Update publishing (9.4)**: new `update_issue`/`update_work_item`/`update_issue` functions on all three connectors (Jira `PUT`, ADO JSON-Patch `replace`, GitHub `PATCH`) alongside the existing `create_issue` ones. Shared `apps/api/app/services/connectors/update_detection.py::find_existing_publication()` walks a `DraftItem`'s `revisionOfId` chain looking for an already-published predecessor on the same tool. All three publish routers branch on this: a targeted-regen revision of an already-published item now **updates that external issue in place** — same key, same URL, the existing `PublishedItem` row re-pointed to the new `DraftItem` (not duplicated) — instead of creating a second issue. New cross-tool `POST /draft-items/{id}/flag-removed` endpoint (+ `add_comment` on all three connectors) posts a comment on the external issue when its source content was removed — explicitly never auto-closing; a "Flag {key} for reviewer" button surfaces this in the delta review UI. Every update-publish and flag-removed action gets its own audit entry with before/after state (`draft_item.publish_updated`, `draft_item.flagged_removed_externally`). **Live-verified with real Jira API calls**: published a real issue (`KAN-4`), then published a revision — confirmed it updated the same issue key in place (not `KAN-5`), with `PublishedItem` correctly re-pointed and the audit trail showing `published` → `publish_updated`.
- **Drift detection (9.5)**: needs "a real job scheduler (none exists yet)" per the issue's own scope note — built as an **on-demand, manually-triggered check** instead of inventing scheduler infrastructure, which still satisfies the acceptance criteria ("detected on the next sync check"). `PublishedItem.lastKnownState`/`lastSyncedAt` snapshot what SpecMate wrote on every create/update publish. New `fetch_issue`/`fetch_work_item`/`fetch_issue` read functions on all three connectors. `POST /projects/{id}/drift-check` (+ web proxy) compares the live remote title/description against `lastKnownState` for every published item in the project; a mismatch creates (or refreshes, never duplicates) a `DriftFlag` row. Reviewer resolution via `POST /drift-flags/{id}/resolve`: **accept external** (SpecMate adopts the remote text as its new baseline, no republish) or **reassert SpecMate's version** (leaves the baseline as-is — the next update-publish will overwrite the drift, an explicit informed choice, never a silent default). Surfaced as a "Check for drift" panel + open-flag list with resolve actions on the project audit page (`/workspaces/{ws}/projects/{p}/audit`). Status (title/workflow-state) is intentionally excluded from the diff — it varies too much per-tool configuration to compare generically; only title/description are.
- **Not built**: an actual cron/scheduler to run drift checks automatically (documented gap, same as the existing metering-job gap in the pricing section above) — the check is manual/on-demand for now, triggered from the audit UI.

### Connector abstraction layer (Issue 11.1, `apps/api/app/services/connectors/types.py` + `transport.py`)

Retrofit onto the three already-shipped, tested publish connectors (Epics 5/6/7) so a future on-prem agent (Epic 11) can relay their HTTP calls without rewriting any connector logic. New `ConnectorTransport` Protocol in `types.py`: one method, `request(method, url, *, auth=None, headers=None, json=None, params=None, timeout=30) -> httpx.Response` — a single request/response round-trip, never a retry loop. It returns the real `httpx.Response` object (not a custom wrapper), so every existing `response.status_code` / `.headers.get(...)` / `.json()` / `.text` call site in `jira_publish.py`/`ado_publish.py`/`github_publish.py` needed zero changes — only the `async with httpx.AsyncClient(...) as client:` open/call boilerplate was replaced by a `transport.request(...)` call. All retry/backoff/status-code-branching/error-body-parsing logic (429/5xx backoff with `Retry-After`, GitHub's 403-as-rate-limit special case, JSON vs. plain-text error bodies) stays exactly where it was, in each connector's own functions.

Two implementations in `transport.py`: `DirectCloudTransport` (the default — opens an `httpx.AsyncClient`, makes one request, closes it; used for Jira Cloud, ADO Services, github.com) and `AgentRelayTransport`, a proven-shape stub that takes an `agent_endpoint` and raises `NotImplementedError` on every call — proving the interface can accommodate a future relayed call through an on-prem agent without any connector rewrite, without pretending relay actually works yet (Issues 11.2/11.3, not built).

Every function in the three `*_publish.py` files that used to open `httpx.AsyncClient` directly now takes an optional `transport: ConnectorTransport` parameter defaulting to a module-level `DirectCloudTransport()` instance, so nothing about existing callers changes. `PublishGateway`/`AdoPublishGateway`/`GitHubPublishGateway` (`apps/api/app/routers/publish*.py`) each carry a `transport: ConnectorTransport` field alongside their existing injectable callables — the same DI seam the routers already use for tests, ready for a future on-prem-agent flow to swap in `AgentRelayTransport` without touching router logic. Today's router call sites don't pass `transport=` explicitly; the connector functions' own default keeps behavior identical to pre-refactor. Read-side connectors (`jira.py`/`ado.py`/`github.py`, Epic 2's backlog-sync) are explicitly out of scope — structurally similar but code-independent, not named in the issue.

### Agent relay protocol spec (Issue 11.2, documentation only — [`docs/agent-relay-protocol.md`](docs/agent-relay-protocol.md))

Design doc, no code — the message contract a future `AgentRelayTransport` (11.1's stub) will speak once Epic 11 is actually built. Covers the auth handshake (agent registers once via a workspace-admin-issued token, then signs every request with a locally-held keypair cloud never sees; the agent's _local_ credential to Jira/ADO/GitHub Server stays entirely on-prem, cloud never sees that either), the item-push message format (mirrors `ConnectorTransport.request()`'s existing parameters 1:1 so the eventual cloud-side shim stays a thin serialize/deserialize layer), the status-callback format (echoes raw `http_status`/`response_body` back so existing connector parsing logic needs no changes), and error propagation (distinguishes "remote tool rejected it" from "agent itself failed" from "agent never responded," each surfacing differently in the publish UI). Recommends long-polling over WebSocket or a message queue for the eventual transport — no new infra, works through the outbound-only firewalls on-prem customers actually have, consistent with this repo's existing "Postgres job table, no broker" approach to async work. Not a commitment — the eventual build issue should confirm before implementing.

### Agent relay security model (Issue 11.3, documentation only — [`docs/agent-relay-security-model.md`](docs/agent-relay-security-model.md))

Companion to 11.2, written to double as a security-review handout for a prospective on-prem customer. Core invariants: the agent is **outbound-only** — it never listens on any port, so no inbound firewall rule or DMZ placement is ever required on the customer's side; **TLS 1.2+ is the floor**, with mutual TLS recommended as the target (the agent's registration bootstrap generates a client cert, not just a signing keypair, and SpecMate cloud's ingress verifies it for the `/agents/*` route family specifically — flagged as a real infra lift to scope explicitly when Epic 11 is built, not something that falls out "for free"); and **credential isolation** is stated as five hard invariants — the agent holds only the on-prem tool's own credential and never transmits it to SpecMate cloud in any form, is scoped to exactly one `(workspace, publish mapping)` pair per registration, can reach nothing on SpecMate cloud beyond the two relay-protocol routes, and SpecMate cloud holds no standing secret that could reach back into the customer's network — with revocation instant and cloud-side, independent of the agent process being reachable. Closes with a reviewer-facing checklist (network egress verification, credential-scope review, log-hygiene check, revocation test) meant to be handed to a customer's security team as-is.

Net-new test coverage in `apps/api/tests/services/test_transport.py` (`DirectCloudTransport` forwards method/url/auth/headers/json/params to a mocked `httpx` transport unchanged; `AgentRelayTransport` always raises). All 3 existing publish-router test files pass **unchanged** — proof the retry/backoff logic didn't shift shape during the refactor.

### API rate limiting (Issue 12.1, `apps/api/app/core/rate_limit.py` + `rate_limit_config.py`)

Per-workspace, tier-based rate limiting on SpecMate's FastAPI backend — scoped to the API surface that exists today (no partner-API-key system invented; see `docs/superpowers/specs/2026-07-20-api-rate-limiting-design.md` for why). A `RateLimitMiddleware` (Starlette `BaseHTTPMiddleware`, installed via `install_rate_limit_middleware()` in `main.py`) resolves workspace identity by parsing `request.url.path` directly — **not** `request.path_params`, which Starlette leaves empty until routing runs _inside_ `call_next()`, a genuine bug caught during implementation before it shipped (confirmed empirically: a `BaseHTTPMiddleware.dispatch()` reading `request.path_params` before `call_next()` always sees `{}`). Three route shapes resolve to a workspace: `^/projects/([^/]+)` (direct `Project.workspaceId` lookup), `^/sources/([^/]+)` (`Source.projectId` → `Project.workspaceId`, covering document-parsing routes like `POST /sources/{id}/parse`, `GET /sources/{id}/diff`, `POST /sources/{id}/targeted-regenerate`), and `^/draft-items/([^/]+)` (`DraftItem.projectId` → `Project.workspaceId`, covering AI-generation routes like `POST /draft-items/{id}/regenerate` and `POST /draft-items/{id}/flag-removed`) — the latter two were added after a whole-feature review found document-parsing and AI-generation endpoints, the highest abuse-risk routes in the system, were shipping completely unrated. Counters live in a new Postgres table, `ApiRateLimitCounter` (`workspaceId`, `windowStart` truncated to the minute, `requestCount`), incremented atomically via a single `INSERT ... ON CONFLICT DO UPDATE ... RETURNING` — no read-then-write race, verified under real concurrent Postgres access. Its FK to `Workspace` cascades on delete (a follow-up fix — the original `RESTRICT` FK broke every pre-existing test that deletes its fixture workspace in cleanup, since the middleware now writes a counter row on every request; cascade is also the semantically correct behavior since a deleted workspace's rate-limit counters are meaningless). Limits are keyed off `Workspace.pricingTier`: 60 req/min for `STARTER`, 600 req/min for `ENTERPRISE` (`rate_limit_config.py`); `requests_per_minute_for_tier()` raises a clear `ValueError` (not an opaque `KeyError`) if a tier is ever added without a matching limit, rather than 500ing every request for that tier. Every response carries `X-RateLimit-Limit`/`X-RateLimit-Remaining`/`X-RateLimit-Reset`; exceeding the limit short-circuits before the route handler runs, returning `429` with a `Retry-After` header and a `{"error": "rate_limited", ...}` body. Old counter rows aren't cleaned up automatically — same documented gap as the metering-job and drift-check schedulers elsewhere in this doc.

**Known gaps (routes still unrated).** A handful of route shapes are explicitly not covered, because none of them carry a workspace-resolvable identifier in their URL path without either a join deeper than the two-hop pattern above or inspecting the request body/query — this middleware does neither: (1) `/drift-flags/{drift_flag_id}/resolve` would need a three-hop chain (`DriftFlag.publishedItemId` → `PublishedItem.draftItemId` → `DraftItem.projectId` → `Project.workspaceId`), deferred as too deep for this pass; (2) the connector sync endpoints — `/connectors/{tool}/reference-items/sync`, `/connectors/confluence/pages/{page_id}/sync`, `/connectors/slack/channels/{channel_id}/sync` — key on a tool name or external page/channel id, neither of which maps to a workspace without extra lookups this middleware doesn't perform (tracked as a follow-up, [Issue #104](https://github.com/nexgen-tech-labs/specmate.io/issues/104), since these fan out to external Jira/ADO/GitHub/Confluence/Slack APIs and carry a partner-relationship abuse risk beyond SpecMate's own capacity); (3) the connection-health/discovery checks — `/connectors/jira/health`, `/connectors/jira/projects`, `/connectors/ado/health`, `/connectors/ado/projects`, `/connectors/github/publish-health`, `/connectors/github/repos` — have no project/workspace identifier in the path at all, relying instead on server-side configured credentials; (4) `/ai/demo-extract` and `/billing/meter-usage` have no path-based identifier whatsoever. Closing (1)/(3)/(4) would require either extending route path shapes to carry a project/workspace id, or teaching the middleware to inspect request bodies before `call_next()`.

### Interactive guided product tour (`apps/web/src/components/tour/`, `apps/web/src/lib/tour-steps.ts`)

An on-demand, hand-rolled spotlight/tooltip overlay (no new dependency, aside from `@testing-library/jest-dom` added as a dev-only test-infra dependency) that walks a user through the real end-to-end flow — create a project, the get-started wizard's connect/upload/generate steps, then review/publish — by highlighting real, already-rendered UI elements marked with `data-tour="..."` attributes. `TourProvider` (React Context, mounted in the root layout) tracks the active step in `sessionStorage` plus a `?tour=<stepId>` URL query param, so state survives full-page navigation between the dashboard, get-started, and review routes without any backend/DB involvement — this is purely client-side, ephemeral UI guidance, not a tracked/resumable-across-devices feature. `TourOverlay` renders nothing unless the active step's target element actually exists on the current page (found via `document.querySelector`), so a step "waits" safely across a navigation gap rather than erroring; it also clamps its tooltip position to stay within the viewport (flipping above the target when there's insufficient room below) and follows the WAI-ARIA dialog pattern (`role="dialog"`, `aria-labelledby`, Escape-to-dismiss, initial focus placement). Triggered via a "Take a tour" button on the workspace dashboard, visible to all roles — not automatic after onboarding. Drives the user's real workspace: real source upload, real AI generation, real review actions, real publish — no seeded demo data or stubbed calls.
...
Authorization: Hierarchical RBAC across Organization → Workspace → Team → User (Issues 12.10/12.11, superseding the flat model from Issue #2). Org roles `OWNER`/`ADMIN` cascade to an effective Workspace `ADMIN` in every workspace of the org; workspace roles remain `ADMIN`/`REVIEWER`/`VIEWER`; project-scoped Teams _restrict_ (never grant) a non-admin member's visibility to their teams' projects. All checks go through `requireWorkspaceRole`/`requireProjectRole` (`apps/web/src/lib/workspace-context.ts`) server-side.
...
- ~~Pricing & packaging (Issue 10.9)~~ — done: STARTER (self-serve, Stripe Checkout subscription + metered per-published-item overage via the Billing Meter Events API) / ENTERPRISE (sales-assisted, no Stripe object) tiers; signature-verified Stripe webhook syncing subscription state; idempotent usage metering (`UsagePeriod`, one row per workspace per UTC month) + delta-based Stripe usage reporting; margin-flagging dashboard extension using real tier-derived revenue instead of only the manual `planRevenueUsd` fallback. **Amended**: tier selection is no longer forced at signup — every new workspace is free while solo (1 member, no Stripe touch), and billing is gated at invite-acceptance time (`checkSeatGate`, 402 `BILLING_REQUIRED`) the moment a workspace would gain a second member; org-role-derived implicit access never counts as a seat. **Placeholder pricing** (no real price points decided) and **no real Stripe account exists yet** — all of this is built and tested against Stripe's API shape (incl. signature verification with `generateTestHeaderString`) but has never run against a live test-mode Stripe account. Deferred: marketplace-native billing (Issues 10.2–10.4), a real job scheduler for periodic metering.
- ~~Atlassian Connect app framework (Issue 10.2, technical piece)~~ — done: multi-tenant install lifecycle (`AtlassianConnectInstall`, descriptor, trust-on-first-use `installed` + JWT-verified `uninstalled` webhooks, workspace-claim flow), QSH-verified request auth on both sides (`@atlassian/atlassian-jwt` on web, a hand-implemented-and-cross-verified Python equivalent on api), and a second `JiraConnection` implementation (`ConnectJwtConnection`, 2-Legged JWT). Live-verified end-to-end against the real dev DB (install → unclaimed row → correctly-signed uninstall → verified). Not built: the actual Marketplace listing/submission (needs a live Atlassian Developer account + business review — out of engineering scope), and wiring `ConnectJwtConnection` into `publish.py`'s still-single-tenant `PublishGateway` (deliberately left for the deferred per-workspace connection store, Issue 5.1).
- ~~Organization → Workspace → Team → User hierarchy (Issues 12.10/12.11)~~ — done, additively: org tables + role inheritance (org OWNER/ADMIN ⇒ effective workspace ADMIN), restriction-based team project scoping enforced server-side across all 25 project-scoped routes/pages via the new `requireProjectRole`, backfill wrapping every pre-hierarchy workspace in an implicit single-workspace org (verified 21/21), signup creating the full chain, team + org-workspace management endpoints. Deferred: the management UI (Issue #99), org-level billing consolidation (decision recorded on #97), Team-level notifications (Teams are permission groups only so far).
