...
**Known gaps (routes still unrated).** A handful of route shapes are explicitly not covered, because none of them carry a workspace-resolvable identifier in their URL path without either a join deeper than the two-hop pattern above or inspecting the request body/query — this middleware does neither: (1) `/drift-flags/{drift_flag_id}/resolve` would need a three-hop chain (`DriftFlag.publishedItemId` → `PublishedItem.draftItemId` → `DraftItem.projectId` → `Project.workspaceId`), deferred as too deep for this pass; (2) the connector sync endpoints — `/connectors/{tool}/reference-items/sync`, `/connectors/confluence/pages/{page_id}/sync`, `/connectors/slack/channels/{channel_id}/sync` — key on a tool name or external page/channel id, neither of which maps to a workspace without extra lookups this middleware doesn't perform; (3) the connection-health/discovery checks — `/connectors/jira/health`, `/connectors/jira/projects`, `/connectors/ado/health`, `/connectors/ado/projects`, `/connectors/github/publish-health`, `/connectors/github/repos` — have no project/workspace identifier in the path at all, relying instead on server-side configured credentials; (4) `/ai/demo-extract` and `/billing/meter-usage` have no path-based identifier whatsoever. All four categories pass through the middleware unrated today; closing them would require either extending route path shapes to carry a project/workspace id, or teaching the middleware to inspect request bodies before `call_next()`.
