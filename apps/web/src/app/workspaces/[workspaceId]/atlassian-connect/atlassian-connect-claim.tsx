...
export function AtlassianConnectClaim({
  workspaceId,
  initialClaimed,
  initialUnclaimed,
}: {
  ...
  async function claim(installId: string) {
    ...
    const res = await fetch(`/api/workspaces/${workspaceId}/atlassian-connect`, {
      method: 'POST',
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-sm font-semibold text-ink">Linked Jira sites</h2>
        {initialClaimed.length === 0 ? (
          <p className="mt-2 text-sm text-sub">No Jira site is linked to this workspace yet.</p>
        ) : (
          <ul className="mt-3 divide-y divide-line rounded-lg border border-line bg-panel">
            {initialClaimed.map((install) => (
              <li key={install.id} className="flex items-center justify-between px-4 py-3">
                <span className="text-sm text-ink">{install.displayUrl ?? install.baseUrl}</span>
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-sub">
          A Jira admin installed SpecMate from the Atlassian Marketplace but hasn&apos;t linked it
          to a workspace yet. Claim the site below if it belongs to your team.
        </p>
