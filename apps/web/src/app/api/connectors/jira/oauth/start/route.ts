...
// "Connect with Jira" button can't navigate the browser straight to
...
// generalized for Jira as a fast-follow).
export async function GET(request: Request) {
  ...
  const target = `${process.env.API_BASE_URL}/connectors/jira/oauth/start?wizard_session_id=${encodeURIComponent(wizardSessionId)}`;
