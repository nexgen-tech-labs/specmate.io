...
// Connector setup is an ADMIN concern (Issue 5.3) — creating a wizard session
// starts the guided setup flow, so it's gated the same as every other
// connector-setup route (Issue #101).
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    const response = await fetch(
      `${process.env.API_BASE_URL}/workspaces/${workspaceId}/projects/${projectId}/wizard-sessions`,
      {
        method: 'POST',
