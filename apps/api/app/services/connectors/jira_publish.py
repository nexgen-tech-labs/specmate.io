...
async def discover_projects(connection: JiraConnection) -> list[dict[str, str]]:
...
async def discover_project_meta(
    connection: JiraConnection, project_key: str
) -> dict[str, object]:
    ...
    async with httpx.AsyncClient(auth=connection.auth(), timeout=30) as client:
        try:
            types_response = await client.get(f"{base}/issue/createmeta/{project_key}/issuetypes")
